...
data_train %>% 
    ggplot(aes(x= Lot.Area)) + geom_boxplot(fill = '#006EA1', color = 'black') +
...
data_train %>% 
    ggplot(aes(x= Total.Bsmt.SF)) + geom_boxplot(fill = '#006EA1', color = 'black') +
...
data_train %>% 
    ggplot(aes(x= Garage.Area)) + geom_boxplot(fill = '#006EA1', color = 'black') +
...
```{r}
noutlier <- data_train %>% 
    filter(Garage.Area > (Q1Garage - 1.5*IQRGarage) & Garage.Area < (Q3Garage + 1.5*IQRGarage))
